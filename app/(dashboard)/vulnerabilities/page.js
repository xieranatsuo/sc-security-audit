...
const SEVERITY_COLORS = { critical: 'text-red-400 bg-red-500/10 border-red-500/20', high: 'text-orange-400 bg-orange-500/10 border-orange-500/20', medium: 'text-yellow-400 bg-yellow-500/10 border-yellow-500/20', low: 'text-indigo-400 bg-indigo-500/10 border-indigo-500/20', informational: 'text-gray-400 bg-gray-500/10 border-gray-500/20' };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-5 gap-3">
        {[
          ...
          { label: 'Low', count: VULNERABILITIES.filter(v => v.severity === 'low').length, color: 'text-indigo-400' },
        ].map(s => (
          ...
          <div key={v.id} className="rounded-lg bg-surface-1 border border-gray-600/10 overflow-hidden">
            ...
            {expanded === v.id && (
              <div className="px-5 pb-5 border-t border-gray-600/10 pt-4 space-y-4 animate-fade-in">
                ...
                <div>
                  <p className="text-indigo-400 text-xs font-semibold mb-1">Detection Logic</p>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {v.references.map((ref, i) => (
                      <a key={i} href={ref} target="_blank" rel="noopener noreferrer" className="text-indigo-400 text-xs hover:underline break-all">{ref.replace('https://', '')}</a>
                    ))}
                  </div>
                </div>
              </div>
      ...
      <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-lg p-4">
        ...
        <p className="text-gray-400 text-xs">This database provides comprehensive vulnerability reference. Vulnerability patterns may evolve. Powered by automated pattern matching — results generated by analysis engine.</p>
      </div>
    </div>
  );
}
