...
const navigation = [
  {
    section: 'Audit',
    items: [
      { name: 'Single Audit', href: '/audit', icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' },
  ...
  },
  {
    ...
    items: [
      { name: 'Block Explorer', href: '/explorer', icon: 'M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9' },
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'Changelog', href: '/changelog', icon: 'M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z' },
...
export function Sidebar() {
  ...
  return (
    <aside className={`fixed left-0 top-0 h-screen bg-surface-1 border-r border-gray-600/10 flex flex-col transition-all duration-300 z-50 ${sidebarCollapsed ? 'w-[68px]' : 'w-64'}`}>
      {/* Logo */}
      <div className="h-16 px-4 flex items-center justify-between border-b border-gray-600/10 shrink-0">
        ...
        <button
          ...
          className={`w-6 h-6 rounded flex items-center justify-center text-gray-500 hover:text-white hover:bg-surface-2 transition-colors shrink-0 ${sidebarCollapsed ? 'mx-auto mt-2' : ''}`}
        >
          ...
          <div key={group.section} className="mb-1">
            {!sidebarCollapsed && (
              ...
              >
                ...
                <svg
                  className={`w-3 h-3 text-gray-600 transition-transform ${collapsed[group.section] ? '-rotate-90' : ''}`}
      ...
      {/* Mode Indicator */}
      {!sidebarCollapsed && (
        <div className="px-3 py-2 border-t border-gray-600/10">
          <div className="flex items-center gap-2 px-2 py-1.5 rounded bg-surface-2/50">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-[10px] text-gray-400">Live Mode</span>
            <span className="ml-auto text-[10px] text-gray-500 font-mono">v1.0.0</span>
          </div>
        </div>
      )}
      ...
      <div className="px-3 py-3 border-t border-gray-600/10 shrink-0">
        {!loading && user ? (
          sidebarCollapsed ? (
            <button onClick={logout} className="w-8 h-8 rounded-lg bg-surface-2 flex items-center justify-center mx-auto hover:bg-surface-3 transition-colors" title="Sign Out">
            ...
            <div className="space-y-2">
              ...
              <button onClick={logout} className="w-full flex items-center justify-center gap-2 px-3 py-1.5 rounded-lg text-xs text-gray-400 hover:text-white hover:bg-surface-2 transition-colors">
